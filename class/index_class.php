...
<?php
 class index
 {
    ...
    public function get_loaisanpham($loaisanpham_id, $sort = '') {
        ...
        if ($sort == 'asc') {
            ...
            $query .= " ORDER BY CAST(sanpham_gia AS UNSIGNED) ASC";
        } elseif ($sort == 'desc') {
            // Sắp xếp theo giá từ cao đến thấp
            $query .= " ORDER BY CAST(sanpham_gia AS UNSIGNED) DESC";
        } else {
            // Sắp xếp mặc định theo ID sản phẩm giảm dần
            $query .= " ORDER BY tbl_sanpham.sanpham_id DESC";
        }
   ...
   public function getCurURL()
{
    ...
    public function insert_payment($session_idA,$deliver_method,$method_payment,$today) {
        ...
        if($result==null) {
            ...
            $query = "INSERT INTO tbl_carta (sanpham_anh,session_idA,sanpham_id,sanpham_tieude,sanpham_gia,quantitys,sanpham_size) VALUES 
             ('$sanpham_anh','$session_idA','$sanpham_id','$sanpham_tieude','$sanpham_gia','$quantitys','$sanpham_size')";
