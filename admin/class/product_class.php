...
<?php
 class product
 {
    ...
    public function insert_product($data,$file){
              ...
              $query = "INSERT INTO tbl_sanpham (sanpham_tieude,sanpham_ma,danhmuc_id,loaisanpham_id,sanpham_gia,sanpham_chitiet,sanpham_baoquan,sanpham_anh) 
              VALUES 
    ...
    public function show_order() {
        $query = "SELECT DISTINCT tbl_order.*, tbl_payment.*, tbl_diachi.*
        FROM tbl_order 
...
public function search_product($keyword) {
    $query = "SELECT tbl_sanpham.*, tbl_danhmuc.danhmuc_ten, tbl_loaisanpham.loaisanpham_ten
              ...
              INNER JOIN tbl_loaisanpham ON tbl_sanpham.loaisanpham_id = tbl_loaisanpham.loaisanpham_id
              WHERE tbl_sanpham.sanpham_tieude LIKE '%$keyword%'
              ORDER BY tbl_sanpham.sanpham_id DESC";
    $result = $this->db->select($query);
    return $result;
}

public function search_orderAll($keyword){
    $query = "SELECT DISTINCT tbl_order.*, tbl_payment.*, tbl_diachi.*
              FROM tbl_order 
              INNER JOIN tbl_payment ON tbl_order.session_idA = tbl_payment.session_idA
              INNER JOIN tbl_diachi ON tbl_order.customer_xa = tbl_diachi.ma_px
              WHERE tbl_order.customer_name LIKE '%$keyword%' 
                 OR tbl_order.customer_phone LIKE '%$keyword%'
                 OR tbl_order.session_idA LIKE '%$keyword%'
              ORDER BY tbl_payment.payment_id DESC";
    $result = $this->db->select($query);
    return $result;
}
